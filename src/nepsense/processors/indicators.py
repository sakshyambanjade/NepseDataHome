...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # VWAP Proxy (Turnover / Volume)
    df["vwap"] = (df["turnover"] / df["volume"]).replace([np.inf, -np.inf], np.nan)
    
    # Sector Relative Strength (if sector info available)
    # This is better done in compute_all_indicators but we can prep the column
    df["sector_ret_1d"] = 0.0 # Placeholder to be filled in compute_all
    df["relative_strength_1d"] = 0.0
    
    # --- Market Regime Detection ---
    # Simple Volatility-Trend Regime
    # 0: Low Vol Bull, 1: High Vol Bull, 2: Low Vol Bear, 3: High Vol Bear
    vol_high = df["vol_20"] > df["vol_20"].rolling(60).median()
